...
While browsing the application as an unauthenticated user, we can notice that some pages contained an Edit link. Clicking the link redirected to the `/login` page, indicating that authentication was required through the normal interface.
Intercepting the request using Burp Suite allows us to inspect the edit endpoint:
`/page/edit/1`

Instead of requesting the page normally, we can change the HTTP method to `OPTIONS`.
...
Since POST was allowed on the endpoint, sending a POST request directly to:
`/page/edit/1`

bypasses the normal login flow without authenticating.
...
Next, we can investigate the `/login` endpoint.

Submitting a normal, nonexistent username resulted in:
`Unknown User`
We can then test whether the username parameter was vulnerable to SQL injection. Supplying: `admin'`caused the application to return: `500 Server Error`
...
Testing with: `admin' OR '1'='1'#`
Instead of receiving Unknown User, the application responded with:
`Invalid Password` This was significant because it indicated that the SQL query was being affected by our input.
...
We can then use a `UNION` injection:
...
It is important to understand that: `SELECT 1234;`
does not mean: `Find 1234 in a table.`
...
However: `SELECT 1234;` can still return:
...
We can think of it roughly as asking the database: `Give me the value 1234.`
...
We can notice that the application produced different responses depending on whether the injected SQL condition matched a user.

For example, testing: `admin' OR username LIKE "a%"` The application responded with: `Unknown User`, Testing further with: `admin' OR username LIKE "d%"` yields: `Invalid Password`, This gives us a useful true/false SQL condition.
...
The SQL condition: `username LIKE 'd%'`

means:
Does a username exist that starts with d? The % wildcard means that any characters can follow d. Once d is confirmed, we can try:
...
Doing this manually would be tedious, so the process can be automated with Python:
...
- The script uses two nested loops. The outer loop determines how many characters we want to discover: `for position in range(10):`

- The inner loop tries every character in our alphabet: `for letter in alphabet:`

- The current guess is constructed with: `guess = username + letter`
